...
function solve_spacecraft(prob; k=25, s=missing)
    ...
    sol12 = solve(prob,
                tspan=(0.0, 200.0),
                alg=TMJets(abs_tol=1e-5, max_steps=10_000, orderT=5, orderQ=1, disjointness=RA.BoxEnclosure()),
                max_jumps=1,
                intersect_source_invariant=false,
                intersection_method=RA.TemplateHullIntersection(boxdirs),
                clustering_method=RA.LazyClustering(1),
                disjointness_method=RA.BoxEnclosure())

    sol12jump = overapproximate(sol12[2](120 .. 150), Zonotope)
    t0 = tstart(sol12jump[1])
    sol12jump_c = cluster(sol12jump, 1:length(sol12jump), RA.BoxClustering(k, s))
